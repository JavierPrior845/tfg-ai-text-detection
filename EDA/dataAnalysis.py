...
def perform_eda(file_path):
    ...
    df['avg_word_length'] = df['content'].apply(lambda x: sum(len(word) for word in str(x).split()) / len(str(x).split()) if len(str(x).split()) > 0 else 0)
    ...
    plt.tight_layout()
    plt.savefig("eda_word_distribution.png")
    print("\nGráfico guardado como eda_word_distribution.png")

    # 5. Guardar resumen para la memoria del TFG
    stats.to_csv("eda_summary_stats.csv")
    return df
